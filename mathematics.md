...
In the lens of energy-based models (EBM), energy function in predictive coding (a scalar value) is calculated by 
...
$E^i = (\mathbf{e}^i)^T(\mathbf{e}^i)$ where $\mathbf{e}^i$ is **vector** $(1,d)$

$E = \cdots + (\mathbf{e}^i)^T(\mathbf{e}^i) + (\mathbf{e}^{i+1})^T(\mathbf{e}^{i+1}) + \cdots$

**Vector** $\mathbf{e}^i = (\mathbf{z}^i - \boldsymbol{\mu}^i)$ where dimensions are $(1,d) - (1,d)$
...
**Vector** $\boldsymbol{\mu}^i = f(\mathbf{z}^{i-1})\mathbf{W}^{i-1}$ where dimensions are $(1,d) = (1,d') \cdot (d',d)$ and $\mathbf{W}^{i-1}$ is **matrix**
